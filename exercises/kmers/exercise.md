...
Indeed, `RNACodon` is an alias in Kmers for a length 3 kmer of the alphabet `RNAAlphabet{2}`.

* Q 2.1.1: Create a `Vector` containing all 64 different RNA codons. What is its type?

* Q 2.1.2: Filter away all codons C whose reverse complement is before C when put in alphabetical order. How many are left?
